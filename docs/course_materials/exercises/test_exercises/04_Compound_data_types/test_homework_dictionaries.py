...
def test_countries(helpers):
    ...
    dictionary_message = str(
        "make sure to set all the values in the "
    ...
    helpers.tests_list = [
        ...
        [r"""^.*\s*=\s*\{\s*""", dictionary_message],
        [r""".*\bItaly\b.*\:\s?2\s?,.*""", dictionary_message],
        [r""".*\bSpain\b.*\:\s?3\s?,.*""", dictionary_message],
        [r""".*\bIsrael\b.*\:\s?1\s?.*\}?""", dictionary_message],
