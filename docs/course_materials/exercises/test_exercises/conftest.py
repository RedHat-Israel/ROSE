...
class Test_helpers:
    ...
    def test_assignment(self):
        ...
        assert test_code, LOGGER.info(
            "Good job, but needs " + f"some improvements:\n{message[:-1]}"
        )
        if message != "":
    ...
    def test_file_exist(self):
        ...
        assert os.path.exists(self.student_file), LOGGER.warning(
            "Student homework file " + "not found: " + self.student_file
        )
    ...
    def run_cmd(self, data=[], **kwargs):
        ...
        """
        input_data = "\n".join(data).encode("utf-8") if data else None

        p = Popen(["python", self.student_file], stdout=PIPE, stdin=PIPE, stderr=PIPE)
        try:
        ...
        if p.returncode != 0:
            LOGGER.error(
                "Failed while running" + f" the code. The error is:\n{stderr.decode()}"
            )
