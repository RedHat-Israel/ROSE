...
def test_times(helpers):
    ...
    expected_msg = (
        "You have to spend {} minutes this week to " + "complete ROSE homework"
    )
    helpers.input = [["2", "3"], ["11", "6"], ["10", "2"]]
    helpers.tests_list = [
        [expected_msg.format(6), f"for input: {helpers.input[0]} expected output: 6"],
        [expected_msg.format(66), f"for input: {helpers.input[1]} expected output: 66"],
        [expected_msg.format(20), f"for input: {helpers.input[2]} expected output: 20"],
    ]
...
def test_manipulations(helpers):
    ...
    helpers.tests_list = [
        ...
        [r"""\b[s]\s*=\s*[\'\'\'].*""", "missing '''"],
        [r""".*[\'\'\']""", "expected several lines that end with '''"],
    ]
