...
def process(players, track):
    ...
    for player in sorted_players:
        ...
        if (player.x, player.y) in positions:
            log.info(f'player {player.name} collision at {player.x},{player.y}'
                     )
        ...
        log.info(f'process_actions: name={player.name} lane={player.lane} '
                 f'pos={player.x},{player.y} score={player.score} '
                 f"response_time={player.response_time:.6f}"
                 )
